                        ...
                        @forelse($meetHosts as $meetHost)
                            ...
                            >
                                <x-table.cell class="w-11/12">
                                    <a href="{{ $meetHost->path() }}" class="hover:underline hover:font-bold">
                                        {{ $meetHost->name }}
                                    </a>
                                </x-table.cell>
                                <x-table.cell class="w-1/12 flex justify-end lg:px-2">
                                    <x-dropdown.dropdown>
                                        ...
                                        <x-slot name="content">
                                            <x-dropdown.link wire:click="editRecord({{ $meetHost->id }})">
                                                Edit
                                            </x-dropdown.link>
                                            <x-dropdown.link wire:click="confirmDelete({{ $meetHost->id }})">
                                                Delete
                                            </x-dropdown.link>
                                        </x-slot>
                                    </x-dropdown.dropdown>
                            ...
                            <x-modal.confirmation wire:model.defer="showConfirmModal">
                                <x-slot name="title">Delete Meet Host?</x-slot>
                                ...
                                <x-slot name="footer">
                                    ...
                                    <x-button.danger wire:click="destroy({{ $meetHost->id }})">
                                        Yes, Delete Meet Host
                                    </x-button.danger>
                                </x-slot>
                        ...
                        @empty
                            <x-table.row class="flex w-full">
                                <div class="flex flex-col items-center mx-auto">
                                    <x-icon.user-group />
                                    <h3 class="mt-2 text-sm font-medium text-gray-900">No Meet Hosts</h3>
                                    <div class="mt-6">
                                        <button type="button"
    ...
    <x-modal.dialog wire:model.defer="showFormModal">
        <x-slot name="title">
            <div x-data="{editing: @entangle('editing')}">
                <span x-show="editing === true">Edit Meet Host</span>
                <span x-show="editing === false">Add a Meet Host</span>
            </div>

        </x-slot>

        <x-slot name="content">
            <livewire:properties.meets.meet-host-form />
        </x-slot>
