            ...
            <x-slot name="body">
                @forelse($eventTypes as $eventType)
                    ...
                    >
                        ...
                        <x-table.cell class="w-1/12 flex justify-end lg:px-2">
                            <x-dropdown.dropdown>
                                ...
                                <x-slot name="content">
                                    <x-dropdown.link wire:click="editRecord({{ $eventType->id }})">
                                        Edit
                                    </x-dropdown.link>
                                    <x-dropdown.link wire:click="confirmDelete({{ $eventType->id }})">
                                        Delete
                                    </x-dropdown.link>
                                </x-slot>
                            </x-dropdown.dropdown>
                    ...
                    <x-modal.confirmation wire:model.defer="showConfirmModal">
                        <x-slot name="title">Delete Event Types?</x-slot>
                        <x-slot name="content">Are you sure you want to delete this event type? This cannot be
                            undone.
                        </x-slot>
                        <x-slot name="footer">
                            ...
                            <x-button.danger wire:click="destroy({{ $eventType->id }})">
                                Yes, Delete Event Types
                            </x-button.danger>
                        </x-slot>
                ...
                @empty
                    <x-table.row class="flex w-full">
                        <div class="flex flex-col items-center mx-auto">
                            <x-icon.user-group />
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No Event Types</h3>
                            <div class="mt-6">
                                <button type="button"
    ...
    <x-modal.dialog wire:model.defer="showFormModal">
        <x-slot name="title">
            <div x-data="{editing: @entangle('editing')}">
                <span x-show="editing === true">Edit Event Types</span>
                <span x-show="editing === false">Add a Event Types</span>
            </div>

        </x-slot>

        <x-slot name="content">
            <livewire:properties.events.track.track-event-type-form />
        </x-slot>
