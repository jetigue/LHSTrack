                ...
                <x-slot name="body">
                    @forelse($surfaces as $surface)
                        ...
                        >
                            ...
                            <x-table.cell class="w-1/12 flex justify-end lg:px-2">
                                <x-dropdown.dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown.link wire:click="editRecord({{ $surface->id }})">
                                            Edit
                                        </x-dropdown.link>
                                        <x-dropdown.link wire:click="confirmDelete({{ $surface->id }})">
                                            Delete
                                        </x-dropdown.link>
                                    </x-slot>
                                </x-dropdown.dropdown>
                        ...
                        <x-modal.confirmation wire:model.defer="showConfirmModal">
                            <x-slot name="title">Delete Track Surface?</x-slot>
                            <x-slot name="content">Are you sure you want to delete this track surface? This cannot be
                                undone.
                            </x-slot>
                            <x-slot name="footer">
                                ...
                                <x-button.danger wire:click="destroy({{ $surface->id }})">
                                    Yes, Delete Track Surface
                                </x-button.danger>
                            </x-slot>
                    ...
                    @empty
                        <x-table.row class="flex w-full">
                            <div class="flex flex-col items-center mx-auto">
                                <x-icon.user-group />
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No Track Surfaces</h3>
                                <div class="mt-6">
                                    <button type="button"
    ...
    <x-modal.dialog wire:model.defer="showFormModal">
        <x-slot name="title">
            <div x-data="{editing: @entangle('editing')}">
                <span x-show="editing === true">Edit Track Surface</span>
                <span x-show="editing === false">Add a Track Surface</span>
            </div>

        </x-slot>

        <x-slot name="content">
            <livewire:properties.meets.track.track-surface-form />
        </x-slot>
