            ...
            <x-slot name="body">
                @forelse($genders as $gender)
                    ...
                    >
                        ...
                        <x-table.cell class="w-1/12 flex justify-end lg:px-2">
                            <x-dropdown.dropdown>
                                ...
                                <x-slot name="content">
                                    <x-dropdown.link wire:click="editRecord({{ $gender->id }})">
                                        Edit
                                    </x-dropdown.link>
                                    <x-dropdown.link wire:click="confirmDelete({{ $gender->id }})">
                                        Delete
                                    </x-dropdown.link>
                                </x-slot>
                            </x-dropdown.dropdown>
                    ...
                    <x-modal.confirmation wire:model.defer="showConfirmModal">
                        <x-slot name="title">Delete Gender?</x-slot>
                        <x-slot name="content">Are you sure you want to delete this gender? This cannot be
                            undone.
                        </x-slot>
                        <x-slot name="footer">
                            ...
                            <x-button.danger wire:click="destroy({{ $gender->id }})">
                                Yes, Delete Gender
                            </x-button.danger>
                        </x-slot>
                ...
                @empty
                    <x-table.row class="flex w-full">
                        <div class="flex flex-col items-center mx-auto">
                            <x-icon.user-group />
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No Genders</h3>
                            <div class="mt-6">
                                <button type="button"
    ...
    <x-modal.dialog wire:model.defer="showFormModal">
        <x-slot name="title">
            <div x-data="{editing: @entangle('editing')}">
                <span x-show="editing === true">Edit Gender</span>
                <span x-show="editing === false">Add a Gender</span>
            </div>

        </x-slot>

        <x-slot name="content">
            <livewire:properties.races.gender-form />
        </x-slot>
