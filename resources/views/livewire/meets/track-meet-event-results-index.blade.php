    ...
    <div class="flex w-full justify-between">
        <div class="w-full md:w-3/4 py-4">
            @if ($trackEvent->eventSubtype->eventType->name == 'Running')
                <livewire:meets.track-meet-running-event-results-index :trackEvent="$trackEvent" :trackMeet="$trackMeet"
                                                                       :gender="$gender" />
            @elseif ($trackEvent->eventSubtype->eventType->name == 'Field')
                <livewire:meets.track-meet-field-event-results-index :trackEvent="$trackEvent" :trackMeet="$trackMeet"
                                                                     :gender="$gender" />
            @endif
