    ...
    <div class="flex flex-col">
        ...
        <div x-cloak x-data="{ show: @entangle('showEventsMenu') }" class="">
                    ...
                    <x-slot name="action">
                        ...
                        <div x-show="!show" class="flex mt-6">
                            <x-button.primary wire:click="showMenu">
                                @if ( count($teamResult->trackEvents) > 0)
                                    Edit Events
                                @else
            ...
            <div x-show="show"
                 ...
                 class="flex w-full"
            >
                <div class="flex w-full flex-wrap py-4 bg-gray-100 rounded-lg space-y-4 mb-10">
                                ...
                                @if($eventType->subTypes)
                                    @foreach($eventType->subTypes as $subType)
                                        <div class="w-full lg:w-1/4 items-start">
                                            ...
                                            <ol class="flex-col space-y-1.5">
                                                @foreach($subType->trackEvents->sortBy('distance_in_meters') as $trackEvent)
                                                    <li wire:key="{{ $loop->index }}" class="relative flex px-2">
                                                        <div class="flex items-center h-5">
                                                            <input id="track-event" aria-describedby="track-event"
